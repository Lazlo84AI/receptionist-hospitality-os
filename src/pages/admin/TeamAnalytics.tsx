...
// ─── Member Row ───────────────────────────────────────────────────────────────
type AnalyticsTab = 'services' | 'individual' | 'training';
...
const MemberRow = ({ member, rank }: { member: UserTaskStats; rank: number }) => {
  ...
  return (
    <div
      className="relative flex items-center gap-4 rounded-xl px-4 py-3 transition-all duration-200 cursor-default"
      style={{
        background: hovered ? 'rgba(187,165,122,0.08)' : 'transparent',
        border: `1px solid ${hovered ? CARD_BORDER : 'transparent'}`,
      }}
    ...
    >
      ...
      <span
        ...
        style={{ color: rank <= 3 ? YELLOW : 'rgba(255,255,255,0.3)' }}
      >
        {rank <= 3 ? ['🥇','🥈','🥉'][rank-1] : `#${rank}`}
      </span>
      ...
      <div className="hidden md:flex items-center gap-6 text-xs">
        <div className="text-center">
          <p className="font-bold" style={{ color: YELLOW }}>{member.tasks_created_total}</p>
          <p style={{ color: 'rgba(255,255,255,0.35)' }}>créées</p>
        </div>
        <div className="text-center">
          <p className="font-bold text-green-400">{member.tasks_completed}</p>
          <p style={{ color: 'rgba(255,255,255,0.35)' }}>closes</p>
        </div>
        <div className="text-center">
          <p className="font-bold" style={{ color: GOLD }}>{member.shifts_completed}</p>
          <p style={{ color: 'rgba(255,255,255,0.35)' }}>shifts</p>
        </div>
      </div>
      ...
      <div className="w-24 flex-shrink-0">
        <div className="flex justify-between text-xs mb-1">
          ...
          <span style={{ color: rate > 70 ? '#4ade80' : rate > 40 ? YELLOW : '#f87171' }} className="font-bold">
            {rate}%
          </span>
        </div>
        <div className="h-1.5 rounded-full" style={{ background: 'rgba(255,255,255,0.08)' }}>
          <div
            ...
            style={{
              width: `${rate}%`,
              background: rate > 70 ? '#4ade80' : rate > 40 ? YELLOW : '#f87171',
            }}
          />
        </div>
      </div>
      ...
      {hovered && (
        <div
          className="absolute right-0 top-full mt-1 z-50 rounded-xl shadow-2xl border p-3 text-xs w-52"
...
const BubbleTooltip = ({ active, payload }: any) => {
  ...
  return (
    <div
      className="rounded-xl px-4 py-3 shadow-2xl border text-xs"
      style={{ background: '#0F0C24', borderColor: svcColor + '55', minWidth: 180 }}
    >
...
const CustomBubble = (props: any) => {
  ...
  return (
    <g>
      ...
      <text x={cx} y={cy + 4} textAnchor="middle" fontSize={10} fontWeight="600" fill={svcColor}>
        {initials}
      </text>
    </g>
  );
...
function IndividualBubbleChart({ teamStats, loading }: { teamStats: UserTaskStats[]; loading: boolean }) {
  ...
  // Sync selection when teamStats loads
  useMemo(() => {
    if (teamStats.length > 0 && selected.size === 0) {
      setSelected(new Set(teamStats.map(m => m.staff_id)));
    }
  }, [teamStats.length]);
  ...
  const toggleAll = () =>
    setSelected(prev =>
      prev.size === teamStats.length ? new Set() : new Set(teamStats.map(m => m.staff_id))
    );

  const bubbleData = useMemo(() =>
    teamStats
      .filter(m => selected.has(m.staff_id))
      .map(m => ({
        ...m,
  ...
  const AxisSelect = ({ value, onChange, exclude }: {
    value: BubbleAxisKey;
    onChange: (v: BubbleAxisKey) => void;
    exclude?: BubbleAxisKey[];
  }) => (
    <div className="flex gap-1 flex-wrap">
      {BUBBLE_AXES.filter(a => !exclude?.includes(a.key)).map(a => (
        <button
          key={a.key}
          onClick={() => onChange(a.key)}
          className="px-2.5 py-1 rounded-lg text-xs font-medium transition-colors"
          style={{
            background: value === a.key ? GOLD : 'rgba(255,255,255,0.06)',
            color:      value === a.key ? '#0F0C24' : 'rgba(255,255,255,0.5)',
          }}
        >
          {a.label}
        </button>
    ...
    <div className="mt-4 rounded-xl border" style={{ background: 'rgba(30,26,55,0.85)', borderColor: 'rgba(187,165,122,0.18)' }}>
      ...
      <div className="flex items-center gap-3 px-5 py-4 border-b" style={{ borderColor: 'rgba(187,165,122,0.18)' }}>
        ...
        <span className="ml-auto text-xs" style={{ color: 'rgba(255,255,255,0.3)' }}>
          {selected.size} / {teamStats.length} membres affichés
        </span>
      </div>
      ...
      <div className="p-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-5">
          <div>
            <p className="text-xs mb-2" style={{ color: 'rgba(187,165,122,0.6)' }}>Axe X (horizontal)</p>
            <AxisSelect value={xAxis} onChange={setXAxis} />
          </div>
          <div>
            <p className="text-xs mb-2" style={{ color: 'rgba(187,165,122,0.6)' }}>Axe Y (vertical)</p>
            <AxisSelect value={yAxis} onChange={setYAxis} />
          </div>
          <div>
            <p className="text-xs mb-2" style={{ color: 'rgba(187,165,122,0.6)' }}>Taille bulle</p>
            <AxisSelect value={zAxis} onChange={setZAxis} />
          </div>
        </div>
        ...
        <div className="mb-5">
          <div className="flex items-center gap-2 mb-3">
            ...
            <button
              ...
              style={{
                background: selected.size === teamStats.length ? 'rgba(187,165,122,0.15)' : 'rgba(255,255,255,0.04)',
                color: GOLD,
          ...
          <div className="flex flex-wrap gap-2">
            {teamStats.map(m => {
              ...
              return (
                <button
                  key={m.staff_id}
                  onClick={() => toggle(m.staff_id)}
                  className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl text-xs font-medium transition-all"
                  style={{
                    background: isOn ? svcColor + '20' : 'rgba(255,255,255,0.03)',
                    border: `1px solid ${isOn ? svcColor + '66' : 'rgba(255,255,255,0.08)'}`,
                    color: isOn ? svcColor : 'rgba(255,255,255,0.3)',
                    transform: isOn ? 'scale(1)' : 'scale(0.96)',
                  }}
                >
                  <span
          ...
          <ResponsiveContainer width="100%" height={420}>
            <ScatterChart margin={{ top: 20, right: 30, bottom: 20, left: 10 }}>
              ...
              <XAxis
                ...
                name={BUBBLE_AXES.find(a => a.key === xAxis)?.label}
              ...
              <YAxis
                ...
                name={BUBBLE_AXES.find(a => a.key === yAxis)?.label}
                tick={{ fill: 'rgba(255,255,255,0.35)', fontSize: 10 }}
                axisLine={false} tickLine={false}
        ...
        <div className="flex flex-wrap gap-3 mt-3 justify-center">
          ...
          <div className="flex items-center gap-1.5 text-xs" style={{ color: 'rgba(255,255,255,0.3)' }}>
            <span style={{ color: GOLD }}>⊙</span> Taille bulle = {BUBBLE_AXES.find(a => a.key === zAxis)?.label}
          </div>
        </div>
...
export default function TeamAnalytics() {
  ...
  // ── Team-wide timeseries (tous membres, sans filtre user) ───────────────
  ...
  useEffect(() => {
    supabase
      ...
      .then(({ data }) => {
        if (!data) return;
        // Agréger par period_type + period (somme de tous les membres)
        ...
        // Reconstruire le tableau avec period_type extrait de la clé
  ...
  const serviceBarData = benchmarks.map(b => ({
    ...
    color: SERVICE_COLORS[b.service || ''] || GOLD,
  }));
  ...
  // ── Timeseries area chart (agrégé tous membres) ───────────────────────
  const chartTimeseries = allTimeseries
    .filter(t => t.period_type === periodFilter)
    .map(t => ({
      label: t.period_label,
      Créées: t.tasks_created,
      Closes: t.tasks_completed,
    }));

  // ── Category donut-ish data ───────────────────────────────────────────────
  const categories = [
  ...
  const [activeAnalyticsTab, setActiveAnalyticsTab] = useState<AnalyticsTab>('services');
    ...
    <AdminLayout>
      <div className="p-6 max-w-7xl mx-auto">
        ...
        <div className="mb-8 flex items-end justify-between">
          ...
          {/* Period selector (for timeseries chart) */}
          <div className="flex rounded-lg overflow-hidden border" style={{ borderColor: CARD_BORDER }}>
            {(['day','week','month'] as const).map(p => (
              <button
                ...
                style={{
                  background: periodFilter === p ? GOLD : 'transparent',
                  color:      periodFilter === p ? '#13102B' : 'rgba(255,255,255,0.5)',
                }}
              >
          ...
          {kpis.map(kpi => (
            ...
            >
              ...
              <div>
                ...
                <p className="text-2xl font-bold" style={{ color: loading ? 'rgba(255,255,255,0.2)' : kpi.color }}>
                  {loading ? '—' : kpi.value}
                </p>
              </div>
            </div>
        ...
        >
          {([
            ...
            { id: 'training',   label: 'Training Results',            icon: Award },
          ] as { id: AnalyticsTab; label: string; icon: any }[]).map(({ id, label, icon: Icon }) => (
            <button
              key={id}
              onClick={() => setActiveAnalyticsTab(id)}
              className="flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-medium transition-all duration-200"
              style={activeAnalyticsTab === id
        ...
        {/* ══════════ TAB: SERVICES TASK MANAGEMENT ══════════ */}
        {activeAnalyticsTab === 'services' && (<>
        {/* ── Charts Row 1 ─────────────────────────────────────────────── */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">

          {/* Bar chart — Performance par service */}
        ...
        {/* ══════════ TAB: INDIVIDUAL TASK MANAGEMENT ══════════ */}
        {activeAnalyticsTab === 'individual' && (<>
        {/* ── Charts Row 2 ─────────────────────────────────────────────── */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">

          {/* Area chart — évolution temporelle (span 2) */}
          <div className="lg:col-span-2 rounded-xl border p-5" style={{ background: CARD_BG, borderColor: CARD_BORDER }}>
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-xs ml-1" style={{ color: 'rgba(255,255,255,0.35)' }}>— données utilisateur connecté</span>
            </div>
          ...
          <div className="rounded-xl border p-5" style={{ background: CARD_BG, borderColor: CARD_BORDER }}>
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-xs ml-1" style={{ color: 'rgba(255,255,255,0.35)' }}>all time</span>
            </div>
            {loading ? (
              <div className="space-y-3">
                {[1,2,3,4].map(i => <div key={i} className="h-8 rounded" style={{ background: 'rgba(255,255,255,0.04)' }} />)}
              </div>
            ) : (
              <div className="space-y-3">
                {(() => {
                    ...
                    <div key={c.name}>
                      ...
                      <div className="h-1.5 rounded-full" style={{ background: 'rgba(255,255,255,0.06)' }}>
                        <div
                          ...
                          style={{ width: `${Math.round((c.value/total)*100)}%`, background: c.color }}
                        />
                      </div>
                    </div>
                  ));
                ...
                {categories.length === 0 && (
                  <p className="text-xs text-center py-8" style={{ color: 'rgba(255,255,255,0.2)' }}>Aucune donnée</p>
                )}
              </div>
            )}
          </div>
        ...
        <div className="rounded-xl border" style={{ background: CARD_BG, borderColor: CARD_BORDER }}>
          <div className="flex items-center gap-3 px-5 py-4 border-b" style={{ borderColor: CARD_BORDER }}>
            ...
            <span className="ml-auto text-xs" style={{ color: 'rgba(255,255,255,0.3)' }}>
              {teamStats.length} membres — trié par tâches créées
            </span>
          </div>
          ...
          {loading ? (
            ...
            <div className="p-10 text-center text-sm" style={{ color: 'rgba(255,255,255,0.2)' }}>
              Aucun membre trouvé
            </div>
          ) : (
            <div className="p-3 space-y-1">
              {teamStats.map((member, i) => (
                <MemberRow key={member.staff_id} member={member} rank={i + 1} />
              ))}
            </div>
          )}
        </div>

        {/* ── Individual Cloud Graph ─────────────────────────────────── */}
        ...
        {activeAnalyticsTab === 'training' && (
          ...
          >
            <Award size={48} className="mb-4 opacity-30" style={{ color: '#BBA57A' }} />
            <p className="text-white font-medium mb-1">Training Results</p>
            <p className="text-sm" style={{ color: 'rgba(187,165,122,0.45)' }}>
              Résultats des QCMs, taux de complétion et scores par membre — en construction
            </p>
          </div>
        )}
