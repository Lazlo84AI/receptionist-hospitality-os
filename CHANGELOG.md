...
- Template email "Invite user" personnalise en francais avec signature Thibault / Decoeur Hotels
- Site URL et Redirect URLs deja configures en amont (Vercel prod + localhost:8080)
- Aucun nouveau secret Edge Functions cree (choix `window.location.origin` envoye depuis le front)

**Securite**
- `verify_jwt = true` (par defaut Supabase) : seul un user authentifie peut appeler l'Edge Function
- Pas de check role/service dans l'Edge Function (decision produit : seulement 2-3 Managers dans le staff, gating frontend via `AdminProtectedRoute` suffit)
- `redirectTo` automatiquement valide par Supabase contre la whitelist Auth > URL Configuration

**Gestion d'erreur amelioree**
- `supabase.functions.invoke()` masque le body JSON des erreurs derriere un message generique "Edge Function returned a non-2xx status code". Wrapping ajoute pour lire `fnError.context.json()` et remonter le vrai message (ex: "User already registered", "invalid enum value", etc.) dans la modal.

**Test end-to-end valide**
- Creation d'un membre Receptionist / Collaborator avec un email Gmail perso
- Email recu, lien clique, mot de passe defini, login reussi
- Profile et staff_directory crees en cascade par le trigger `handle_new_user`

**HORS SCOPE (V2 eventuelle)**
- Import CSV en masse de staff
- Gestion de la suppression d'une invitation avant activation
- Relance d'invitation pour un user qui n'a pas encore active
- Stylisation avancee du template email (logo Decoeur, charte graphique)
