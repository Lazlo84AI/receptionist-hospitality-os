...
- **2026-04-24 (suite 2)** : ajout du flow d'invitation administrateur (bouton "Ajouter un membre" sur `/admin/onboarding`). Nouvelle Edge Function `invite-staff` + composant `src/components/admin/CreateStaffMember.tsx` + adaptation de `src/pages/ResetPassword.tsx` pour accepter aussi le flow invite (en plus du recovery existant). Execution prealable des 2 `ALTER TYPE` sur `service_type` pour ajouter les valeurs `'restaurant'` et `'artificial_intelligence'` -> **entrees de dette technique #7 et #11 marquees resolues**. Le flow exploite le pipeline signup existant (trigger `handle_new_user` -> `profiles` + `staff_directory` via `sync_profiles_to_staff_directory`) donc zero impact sur le signup classique `/auth`.
